...
#include "radiosity_integrator.hpp"
#include <materials/matte_material.hpp>
#include <materials/plastic_material.hpp>

namespace RT_ISICG
{
	Vec3f RadiosityIntegrator::Li( const Scene & p_scene,
								 const Ray &   p_ray,
								 const float   p_tMin,
	...
	Vec3f RadiosityIntegrator::_directLighting( const Scene &	  p_scene,
											  const Ray &		p_ray,
	...
	Vec3f RadiosityIntegrator::trace( const Scene & p_scene,
									const Ray &	  p_ray,
									const float	  p_tMin,
									const float	  p_tMax,
									int			  p_nbounces,
									bool		  isInside ) const
	{
		...
		if ( p_scene.intersect( p_ray, p_tMin, p_tMax, hitRecord ) )
		{
			...
			else if ( hitRecord._object->getMaterial()->isTransparent() )
			{
				...
				Ray reflectRay = Ray( hitRecord._point,
