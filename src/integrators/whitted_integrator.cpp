...
namespace RT_ISICG
{
										...
										const float	  p_tMax ) const
	{
		...
		if ( p_scene.intersect( p_ray, p_tMin, p_tMax, hitRecord ) )
		{
			for ( int i = 0; i < p_scene.getLights().size(); i++ )
			{
				...
				// directlighting boucle sur les lumières, pour chaque lance un shadow ray et shade si pas
				// d'intersection
	...
	Vec3f WhittedIntegrator::_directLighting( const BaseLight * light, const HitRecord hitRecord ) const
	{
		...
		else { color = hitRecord._object->getMaterial()->getFlatColor() * lightSample._radiance * cosTheta; }
		// color = hitRecord._object->getMaterial()->getFlatColor() * lightSample._radiance * cosTheta;
		return color;
	}
